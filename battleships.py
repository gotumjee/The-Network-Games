class battleships:
    ...
    def __attack(self, row, column):
        # checks if the userinput is a hit or not and gives the user another
        # turn if it is
        if(self.__usr == 1 and self.__turn == 1):
            ...
            elif(self.__p2_board[row][column] >= 1 and self.__p2_board[row][column] <= 5):
        ...
        if(self.__usr == 1 and self.__turn == 2):
            ...
            elif(self.__p1_board[row][column] >= 1 and self.__p1_board[row][column] <= 5):
                print(
    ...
    def __checkwin(self, userInput=None):  # checks to see if all ships have been struck
        ...
        if(sum1 == 17 and firstRun == 1):
            print("Player 2 wins!")
            firstRun = 0
            return -1

        elif(sum2 == 17 and firstRun == 1):
            print("Player 1 wins!")
            firstRun = 0
            return -1
    ...
    def inputToGame(self, userInput):
        ...
        try:
            ...
            if(self.__boardSetup != 0):
                orientation = str(tempInput[2]).strip()
